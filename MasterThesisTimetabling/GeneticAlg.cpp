...
bool clsGeneticAlg::DoGAScheduling()
{
	...
	//	for (int i=0;i<10;i++)
		...
		*/
		...
		for(int p=0;p<sizePop;p++)
		{
			...
			if(this->arrPopOfSol[p]->numUnFesibleJury ==0)
			{
				this->arrPopOfSol[p]->ResetData();
				this->Mutation(this->arrPopOfSol[p], this->arrPopOfSol);
			}
			else
			{
				this->Mutation_Controled(this->arrPopOfSol[p], this->arrPopOfSol);
			}
	...
	/*	if (sizeReal>0)
		...
		*/
		...
		for(int i=0;i<this->arrPopOfSol.size();i++)
		{
			if(this->arrPopOfSol[i]->ChoosePeriodForJuries() == true)//feasible solution
			{
				this->arrPopOfSol[i]->ChooseRoomForJuries();
...
bool clsGeneticAlg::LocalSearch(clsSolution* &child_Sol)
{
	...
	tempSol->ChooseRoomForJuries();
	tempSol->UpdateFinalState();
	tempSol->CalculateObjectiveValue();
	
	//Compare the original and the best in offspring
	if (CompareTwoSolutions(child_Sol,tempSol)==1)
	{
		tempSol->CopySolution(*child_Sol);
...
void clsGeneticAlg::SelectingBestSolutions()
{
	...
	for(int i=0;i<this->arrPopOfSol.size()-1;i++)
		for(int j=i+1;j<this->arrPopOfSol.size();j++)
			...
			//Remove the last solutions
			vector<clsSolution*> backup_Sol;
			for (int i=0;i<iPopSize;i++)
			{
				clsSolution* tempSol= new clsSolution();
				arrPopOfSol[i]->CopySolution(*tempSol);
				backup_Sol.push_back(tempSol);
			}
			for (int i=arrPopOfSol.size()-1;i>=0;i--)
			{
				delete arrPopOfSol[i];
			}
			arrPopOfSol.clear();
			vector<clsSolution*>().swap(arrPopOfSol);
			arrPopOfSol=backup_Sol;
}
...
int clsGeneticAlg::CompareTwoSolutions(clsSolution* mySol1, clsSolution* mySol2)
{
	...
	if(mySol1->numUnFesibleJury == 0)//or mySol2->numUnFesibleJury == 0
	{
		...
		{
			int objSol1,objSol2;
			objSol1=mySol1->missingTimeslot+mySol1->middleMissingTimeslot+mySol1->numSession;
			objSol2=mySol2->missingTimeslot+mySol2->middleMissingTimeslot+mySol2->numSession;
			if (objSol1<objSol2)
				return 0;
			else
				return 1;
